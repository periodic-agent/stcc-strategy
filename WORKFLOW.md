...
1b. **Canonical text — text/<slug>.txt (Jul 2026, replaces the replace-list audit model).** Every guide ships with its canonical text file: the approved wording of the whole page, generated by `tools/extract_text.py` and committed in the SAME commit as the guide HTML (a guide push without its canonical file is incomplete — extension of Rule 7). Import workflow: build verbatim → the model screens and FLAGS typos/inconsistencies with context → Periodic_agent approves or rejects each flag → approved corrections are applied via the config `"replace"` list at build time (a pattern that matches nothing is now a hard build error) → the built page's extracted text becomes `text/<slug>.txt`. After import the HTML is the source of truth: later corrections edit the HTML and regenerate the canonical file in one approved commit (`python3 tools/extract_text.py <guide>.html -o text/<slug>.txt`). `verify_guide.py` compares the page against the canonical EXACTLY — alterations, insertions, deletions, and reorderings all fail — so no text change can pass verification without its matching canonical update sitting visibly in the same reviewed commit.
