...
**Status:** DONE — 04 Jul 2026. All 12 core guides on library card images (~19 MB base64 removed); boards in img/guides/[captain]/; five-year-mission charts extracted; TBG embedded card art extracted to img/box2/ as JPG (35 cards, seeds the Box 2 scanner library, box2.json still pending). CDN dependency CLOSED — 04 Jul 2026: all 46 remaining CDN card images downloaded (download_box_images.py, run by Periodic_agent), pushed to img/box2/ and img/box3/, and all 6 CDN-linked guides relinked to local files with lazy loading. Zero geekdo references remain. Card database workbook seeded from all card faces: 77/82 rows tagged. Gap list refreshed 25 Jul 2026: the 4 updated-repeat images (Borg Spatial Trajectory, Lirpa, Phasers, Orb of Time) and Tellarites are all present and wired up; remaining image gaps were the 3 updated TBG cards awaiting new scans and 2 Core Common Directives. CLOSED for TBG — 29 Jul 2026: a contributor scan package delivered all 248 To Boldly Go cards (reprints and updated cards included), replacing the 106 guide extractions; img/box2/ is complete. The 2 bot-play Directives are now CLOSED too — 1 Aug 2026: scans arrived in the solo-challenge package, so Conspiracy (1DIR01/2, glory -4) and Reinforce (1DIR02/2, no glory badge, no traits) are complete in box1.json, and Box 2's own printings were added (Reinforce 2DIR02/2• as a reprint, Time is Running Out 2DIR01/2 as a new record with the Surprise trait). No card image is missing anywhere in the library. Box 1 CLOSED — 1 Aug 2026: a second contributor package replaced all 248 scanned Box 1 cards (the earlier set had glare) and added `card_number` and `glory` to box1.json, promo1.json and promo2.json. Those two Directives are now the only missing card images in the whole library; their records carry null card_number and glory until scanned. Untouched by design: solo, vs-picard (no card images).
...
**Status:** DONE — 1 Aug 2026. All 6 rows complete: card_number and glory read from the card faces, traits and icons supplied by Periodic_agent and bucketed against the existing vocabulary (`Lower Decker` and `Ongoing` are other_traits per Box 3 precedent; no novel traits introduced). Only U.S.S. Cabot carries an icon, one Research Skill. Note the 6 images in `img/promo2/` are still web-res guide extractions, the one folder not at the display standard; a scan package would fix that.
